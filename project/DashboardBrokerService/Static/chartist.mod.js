(function(root, factory) {
  ...
  (function (window, document, Chartist) {
    ...
    Chartist.getBounds = function (svg, normalizedData, options, referenceValue) {
      ...
      if(options.step)bounds.step = options.step;
    ...
    Chartist.createXAxis = function (chartRect, data, grid, labels, options) {
      ...
      data.labels.forEach(function (value, index) {
        var interpolatedValue = options.axisX.labelInterpolationFnc(value, index),
          pos = chartRect.x1 + chartRect.width() / (data.labels.length - 1) * index;
...
//        if(!options.axisX.step || ((i % options.axisX.step) == 0) )
    ...
    Chartist.createYAxis = function (chartRect, bounds, grid, labels, offset, options) {
      ...
      bounds.values.forEach(function (value, index) {
        var interpolatedValue = options.axisY.labelInterpolationFnc(value, index),
          pos = chartRect.y1 - chartRect.height() / (bounds.values.length - 1) * index;
    ...
    Chartist.projectPoint = function (chartRect, bounds, data, index) {
      return {
        ...
        y: chartRect.y1 - chartRect.height() * (data[index] - bounds.min) / bounds.range
      };
    };
